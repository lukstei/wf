...
function escapeLabel(text3) {
  return text3.replace(/"/g, "#quot;").replace(/\r?\n/g, "<br/>");
}
...
// src/md-parser.ts
function getInnerText(node2) {
  if (!node2) return "";
  if (typeof node2.value === "string") return node2.value;
  return Array.isArray(node2.children) ? node2.children.map((child) => getInnerText(child)).join("") : "";
}
...
function parseHeading(heading) {
  ...
  const text3 = getInnerText(heading).trim();
  let candidate = stripLeading(text3);
  ...
  if (!match) return { type: "step", title: text3, depth };
...
function sliceNodesMarkdown(markdown, nodes) {
  if (!nodes?.length) return "";
  const start = nodes[0]?.position?.start?.offset;
  const end = nodes[nodes.length - 1]?.position?.end?.offset;
  return typeof start === "number" && typeof end === "number" ? markdown.slice(start, end).trim() : "";
}
function parseBranchSteps(nodes, targetDepth, markdown, defaultTitle, fallback) {
  if (!nodes.length) return [];
  ...
  if (candidateHeadings.length === 0) {
    const text3 = sliceNodesMarkdown(markdown, nodes);
    return [
      {
        type: "step",
        title: defaultTitle,
        instruction: text3 || fallback
      }
    ];
  }
...
function parseSteps(nodes, targetDepth, markdown) {
  if (!nodes?.length) return { steps: [] };
  const candidateHeadings = nodes.map((node2, idx) => ({ node: node2, idx })).filter(
  ...
  const preamble = preambleNodes.length > 0 ? sliceNodesMarkdown(markdown, preambleNodes) : void 0;
  ...
  while (i < sections.length) {
    ...
    if (parsed.type === "if") {
      ...
      const childHeadings = sec.bodyNodes.map((node2, idx) => ({ node: node2, idx })).filter(
      ...
      if (childHeadings.length > 0) {
        ...
        conditionInstruction = sliceNodesMarkdown(markdown, preNodes) || void 0;
        let balance = 0;
        let elseChildIdx = -1;
        ...
        if (i + 1 < sections.length && parseHeading(sections[i + 1].headingNode).type === "else") {
          const text3 = sliceNodesMarkdown(markdown, sec.bodyNodes);
          yesSteps = [
            {
              type: "step",
              title: `${condition} yes`,
              instruction: text3 || "Execute condition true branch"
            }
          ];
          i++;
          ...
          const text3 = sliceNodesMarkdown(markdown, sec.bodyNodes);
          conditionInstruction = text3 || void 0;
        }
      }
      steps.push({
        type: "condition",
...
function parseWorkflowMarkdown(content3, filePath) {
  const { frontmatter, content: mdContent } = parseFrontmatter(content3);
  const nodes = fromMarkdown(mdContent).children || [];
...
function defaultWorkflowResolver(targetPath, workspacePaths) {
  ...
  try {
    const content3 = fs2.readFileSync(resolved, "utf-8");
    if (/\.(md|markdown)$/i.test(resolved)) {
      try {
        const parsed2 = parseWorkflowMarkdown(content3, resolved);
...
// src/shim/egress.ts
function formatEgress(harness, event, response) {
  if (event.type === "stop") {
        ...
        return {
          ...
          stdout: JSON.stringify({
            hookSpecificOutput: {
              ...
              permissionDecisionReason: isDeny ? reason : void 0
            }
          })
        };
      default:
...
function getLatestMessage(transcriptPath) {
  ...
  try {
    ...
    fs3.readSync(fd, buffer, 0, readSize, stat.size - readSize);
    fs3.closeSync(fd);
...
function formatAdvanceReason(summary, nextTargetStep, nextStepIndex, totalSteps, preamble) {
  ...
  const levelStr = nextTargetStep.level > 0 ? ` (Level ${nextTargetStep.level})` : "";
  const reasonParts = [
    `${prefix}: ${label}`,
    summary,
  ...
  reasonParts.push(
    "",
    "INSTRUCTION:",
    isCondition ? nextTargetStep.instruction || `Evaluate condition: "${nextTargetStep.condition}"` : nextTargetStep.instruction ?? ""
  );
  if (isGate) {
    reasonParts.push(
      "",
  ...
  return reasonParts.filter((part) => part !== void 0).join("\n");
}
...
function parseCommand(input) {
  ...
  if (slashToken !== "wf") {
    ...
    const cmdName = slashToken.replace(/^wf-/, "");
  ...
  if (["status", "next", "stop", "help", "run", "show"].includes(firstToken)) {
    const hint = firstToken === "run" ? "Use /wf <file> directly without 'run'." : firstToken === "status" ? "Use /wf-show instead of /wf status." : `Use /wf-${firstToken} instead of /wf ${firstToken}.`;
    return {
      isWfCommand: true,
      error: hint,
...
function run(info, state, command) {
  ...
  const resolver2 = info.workflowResolver || defaultWorkflowResolver;
  const resolved = resolver2(targetPath, info.payload.workspacePaths);
  if (!resolved) {
    return {
      state,
      response: injectSystemMessage(
...
function detectHarness(payload = {}, env = process.env) {
  ...
  if (payload.conversationId !== void 0 && (payload.stepIdx !== void 0 || payload.transcriptPath !== void 0 || payload.invocationNum !== void 0 || payload.artifactDirectoryPath !== void 0 || payload.toolCall !== void 0)) {
    return "agy";
  }
  if (env.CLAUDE_PLUGIN_ROOT || env.CLAUDE_PROJECT_DIR) {
    return "claude";
  }
  if (payload.hook_event_name !== void 0 || payload.tool_name !== void 0 || payload.stop_hook_active !== void 0 || payload.session_id !== void 0) {
    return "claude";
  }
...
// src/shim/normalizer.ts
function stripBom(text3) {
...
function normalizePayload(payload, modeArg, env = process.env) {
  ...
  const isStop = modeArg === "stop" || eventName === "Stop" || payload.stop_hook_active !== void 0 || Boolean(payload.terminationReason && !payload.invocationNum);
  const isTool = modeArg === "tool" || eventName === "PreToolUse" || payload.toolCall !== void 0 || payload.tool_name !== void 0;
  ...
  if (isTool) {
    ...
    toolCall = {
      ...
      args: rawToolCall?.args || payload.tool_input || {}
    };
  }
  const prompt = payload.prompt || payload.userPrompt;
  const terminationReason = payload.terminationReason || payload.stop_reason;
...
async function runShim(modeArg, rawInput, env = process.env) {
  ...
  const transcriptPath = typeof event.rawPayload.transcriptPath === "string" ? event.rawPayload.transcriptPath : void 0;
...
${currentStep?.instruction || ""}`;
  ...
  if (parsed.command === "show") {
    ...
    if (filePath) {
      ...
      writeOut(prompt2);
      return { exitCode: 0, output: prompt2 };
    }
    if (!state) {
